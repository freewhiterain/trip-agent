...
class FoodWorker(TravelWorker):
    ...
    async def run(self, task: ResearchTask, requirement: TravelRequirement) -> WorkerResult:
        query = f"{task.query} {' '.join(requirement.food_preferences)}"
        evidence = (self.knowledge or get_local_knowledge_service()).search_destination(
            requirement.destination, "food", query
        )
        analysis = await analyze_worker_evidence("food", task, requirement, evidence, llm=self.llm)
        return worker_result_from_analysis(task, "food", evidence, analysis)
